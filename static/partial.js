...
!function(G) {
  ...
  _.go = function(v, _fs) {
    ...
    while (f = fs[++i]) {
      if (f._p_lzt) {
        var lazys = [];
        do lazys.push(f);
        while ((f = fs[++i]) && f._p_lzt);
        v = go_lazy(lazys, v && v._mr ? v[0] : v);
        i--;
      } else if (f == __) v = __;
  ...
  function go_lazy(lazys, data) {
    ...
    if (ender._p_lzt == LT) {
      ...
      return true;
    } else if (ender._p_lzt == LL) {
      var tmp = ender, lm;

      if (_.is_mr(tmp)) {
        ender = tmp[0];
        lm = _.isFunction(tmp[1]) ? tmp[1]() : _.clone(tmp[1]);
      }
  ...
  var L = G.L = {};
  L.map = LM; function LM(iter) {
    iter._p_lzt = LM;
    return iter;
  }
  L.filter = LFT;
  function LFT(iter) { iter._p_lzt = LFT; return iter; }
  L.reject = function(iter) { function f(v) { return !iter(v) } f._p_lzt = LFT; return f; };
  L.find = LFI; function LFI(iter) { iter._p_lzt = LFI; return iter; }
  L.some = LS; function LS(iter) { iter._p_lzt = LS; return iter; }
  L.every = LE; function LE(iter) { iter._p_lzt = LE; return iter; }
  L.take = LT; function LT(limit) { return { limit: limit, _p_lzt: LT }; }
  L.loop = LL; function LL(iter) {


    if(arguments.length>1) iter = _.mr(iter, arguments[1]);
    iter._p_lzt = LL;
      return iter;
  }
